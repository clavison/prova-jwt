...
public class UsuarioService {
    ...
    public Usuario buscarPorId(Long id) {
        return usuarioRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Não encontrado o id: " + id));
    }
    ...
    public Usuario atualizar(Long id, Usuario usuarioAtualizado, List<String> roleNames) {
        ...
        if (!isAdmin) {
            Usuario usuarioAutenticado = usuarioRepository.findByLogin(loginUsuarioAutenticado)
                    .orElseThrow(() -> new RuntimeException("Falha ao encontrar dados do usuário autenticado."));

            if (!usuarioAutenticado.getId().equals(id)) {
                throw new AccessDeniedException("Acesso negado: Somente usuários podem alterar os seus próprios dados.");
            }
